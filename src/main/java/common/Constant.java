package common;

public class Constant {
    public static final int STARTING_YEAR = 2000;
    public static final String DEVELOPER = "Memoeslink";
    public static final String DEFAULT_NAME = "name.default";
    public static final String UNKNOWN_NAME = "name.unknown";
    public static final String TEST_NAME = "┤de la ┤Paz";
    public static final String DEFAULT_USERNAME = "username.default";
    public static final String UNKNOWN_USERNAME = "username.unknown";
    public static final String DEFAULT_OCCUPATION = "occupation.default";
    public static final String UNKNOWN_OCCUPATION = "occupation.unknown";
    public static final String ACCENTED_VOWELS = "aàáâãäåāăąǻȁȃạảấầẩẫậắằẳẵặḁæǽeȅȇḕḗḙḛḝẹẻẽếềểễệēĕėęěèéêëiȉȋḭḯỉịĩīĭįiìíîïĳoœøǿȍȏṍṏṑṓọỏốồổỗộớờởỡợōòóŏőôõöuũūŭůűųùúûüȕȗṳṵṷṹṻụủứừửữựyẙỳỵỷỹŷÿýAÀÁÂÃÄÅĀĂĄǺȀȂẠẢẤẦẨẪẬẮẰẲẴẶḀÆǼEȄȆḔḖḘḚḜẸẺẼẾỀỂỄỆĒĔĖĘĚÈÉÊËIȈȊḬḮỈỊĨĪĬĮIÌÍÎÏĲOŒØǾȌȎṌṎṐṒỌỎỐỒỔỖỘỚỜỞỠỢŌÒÓŎŐÔÕÖUŨŪŬŮŰŲÙÚÛÜȔȖṲṴṶṸṺỤỦỨỪỬỮỰYY̊ỲỴỶỸŶŸÝ";
    public static final String ACCENTED_LETTERS = "àèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇØøÅåÆæǼǽǣŒœÐÞαßþðŠšÝýÝÿŽž";
    public static final String[] ROMAN_NUMERALS = {"I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI", "XII", "XIII", "XIV", "XV", "XVI", "XVII", "XVIII", "XIX", "XX", "XXI", "XXII", "XXIII", "XXIV", "XXV", "XXVI", "XXVII", "XXVIII", "XXIX", "XXX"};
    public static final String[] POST_NOMINAL_LETTERS = {"FdA", "FdEng", "FdSc", "BA", "BEd", "BEng", "BLS", "BSc", "BTech", "MChem", "MComp", "MEng", "MMath", "MPhys", "MSci", "DIS", "DIntS", "DPS", "PGCert", "PGDip", "MA", "MBA", "MDes", "MPhil", "MRes", "MSc", "EngD", "PhD", "DLitt", "DSc", "DTech", "DUniv"};
    public static final String[] SUPPORTED_LOCALES = {"ar", "de", "en", "es", "es_MX", "fr", "it", "hi", "ja", "pt", "ru"};
    public static final String[] DEFAULT_COLORS = {"#00FF7F", "#0AAA7F", "#11BCAF", "#3366FF", "#4169E1", "#49AB81", "#52BF90", "#53E3B3", "#54B2A9", "#5588FF", "#5DD897", "#62D0F0", "#66CDAA", "#6D80CF", "#73C6B6", "#747BDB", "#74CEDB", "#74DB8B", "#76D7C4", "#77AAFF", "#7DCEA0", "#7E96D8", "#7ED8C9", "#7F8FB5", "#7FB3D5", "#8281BD", "#82E0AA", "#85C1E9", "#887EDB", "#92A63D", "#949AC7", "#94C7BE", "#96D8A1", "#9843E7", "#99BAD1", "#99CCFF", "#9F9AFF", "#A07CF9", "#A5965F", "#AA9B88", "#ABC0C1", "#B2656A", "#B2BABB", "#B3B267", "#BAD896", "#BB8FCE", "#BFC9CA", "#C096D8", "#C0C158", "#C1C6A0", "#C39BD3", "#C4C4A9", "#C4F062", "#C59FD8", "#C6F525", "#C7A194", "#C7C394", "#C97B7B", "#C9C4A3", "#C9CDFF", "#CCFF00", "#D0AE7B", "#D0B79F", "#D35151", "#D7DBDD", "#D896BE", "#DB7493", "#DB8E74", "#DBBD90", "#DC4646", "#DDCBCB", "#DDDDDD", "#DDF486", "#DE95FF", "#DF7796", "#EDBB99", "#F08080", "#F0B27A", "#F1948A", "#F3F486", "#F49786", "#F4B886", "#F6CCD3", "#FBB57B", "#FDC53E", "#FE5757", "#FF9BB4", "#FFC0CB", "#FFF488", "#FFFDC4", "#FFFF66"};
    public static final String[] USERNAME_PATTERNS = {"#{first}-#{second}", "#{first}-#{second}.#{job}", "#{first}-#{second}.#{job}_#{denominator}#{letter}", "#{first}-#{second}.#{job}_#{denominator}#{number}", "#{first}-#{second}.#{job}_#{year}", "#{first}-#{second}_#{job}", "#{first}-#{second}_#{job}.#{denominator}#{letter}", "#{first}-#{second}_#{job}.#{denominator}#{number}", "#{first}-#{second}_#{job}.#{year}", "#{first}.#{second}-#{job}", "#{first}.#{second}-#{job}_#{denominator}#{letter}", "#{first}.#{second}-#{job}_#{denominator}#{number}", "#{first}.#{second}-#{job}_#{year}", "#{first}.#{second}", "#{first}.#{second}_#{job}-#{denominator}#{letter}", "#{first}.#{second}_#{job}-#{denominator}#{number}", "#{first}.#{second}_#{job}-#{year}", "#{first}.#{second}_#{job}", "#{first}_#{second}-#{job}", "#{first}_#{second}-#{job}_#{denominator}#{letter}", "#{first}_#{second}-#{job}_#{denominator}#{number}", "#{first}_#{second}-#{job}_#{year}", "#{first}_#{second}", "#{first}_#{second}.#{job}-#{denominator}#{letter}", "#{first}_#{second}.#{job}-#{denominator}#{number}", "#{first}_#{second}.#{job}-#{year}", "#{first}_#{second}.#{job}"};

    private Constant() {
    }
}

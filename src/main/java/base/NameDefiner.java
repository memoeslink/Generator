...
public interface NameDefiner extends common.NameDefiner {
    ...
    default String getPatternName(Randomizer r) {
        ...
        for (char c : namePattern.toCharArray()) {
            switch (c) {
                case 'c' -> sb.append(ResourceGetter.with(r).getChar(english.Constant.LOWERCASE_CONSONANTS));
                case 'e' -> sb.append(ResourceGetter.with(r).getChar(Constant.LOWERCASE_ENDING_CONSONANTS));
                ...
                case 'ɔ' -> sb.append(ResourceGetter.with(r).getString(Constant.EXTENDED_CONSONANT_PAIRS));
                case 'm' -> sb.append(ResourceGetter.with(r).getString(Constant.MIDDLE_CONSONANTS));
                case 'q' -> sb.append(ResourceGetter.with(r).getString(Constant.STARTING_CONSONANTS));
                case 'v' -> sb.append(ResourceGetter.with(r).getChar(english.Constant.LOWERCASE_VOWELS));
    ...
    default String getCompositeUsername(String a, String b, Randomizer r) {
        ...
        switch (r.getInt(6)) {
            case 0 ->
                    username = username + Separator.SPACE.getCharacter() + (common.Constant.STARTING_YEAR + r.getInt(-100, 101));
            case 1 -> {
                int extent = 10;
        ...
        switch (r.getInt(3)) {
            case 0 -> username = StringHelper.remove(username, String.valueOf(Separator.SPACE.getCharacter()));
            case 1 -> {
    ...
    default String getDerivedUsername(String s, Randomizer r) {
        ...
        username = ResourceGetter.with(r).getChar(english.Constant.UPPERCASE_ALPHABET) + username;
    ...
    default String getPatternUsername(String a, String b, Locale locale, Randomizer r) {
        ...
        if (StringHelper.isNullOrBlank(a) || StringHelper.isNullOrBlank(b))
            return common.Constant.DEFAULT_USERNAME;
        a = StringHelper.normalizeAlpha(a).toLowerCase();
        b = StringHelper.normalizeAlpha(b).toLowerCase();
        String pattern = ResourceGetter.with(r).getString(common.Constant.USERNAME_PATTERNS);
        return AlephFormatter.str(pattern)
                ...
                .arg("letter", ResourceGetter.with(r).getChar(english.Constant.UPPERCASE_ALPHABET))
                .arg("number", r.getInt(1, 10))
                .arg("year", common.Constant.STARTING_YEAR + r.getInt(-100, 101))
                .fmt();
    }
}
